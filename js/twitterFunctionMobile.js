    ...
    getTwitterDataAndAppend: function (searchTerm) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, xhr) {
                ...
                $(".widgetHeader:last").append("<h3>" + searchTerm + "</h3>");
        ...
        at: function (tweet) {
            return tweet.replace(/\B[@＠]([a-zA-Z0-9_]{1,20})/g, function (m, username, user) {
                return '<span class="linkWrapper"><a hashname="' + username + '"target="_blank" class="twtr-atreply hashTag atTag" href="http://twitter.com/intent/user?screen_name=' + username + '">@' + username
		  						 + '</a></span><i class="icon-circle-arrow-right icon-large arrowIcon"></i>';
            });
        },
...
$(document).bind('pageinit', function () {
            ...
            $('.linkWrapper').each(function (index, element) {
                ...
                $arrowElement.live('swiperight', function (event) {
                    alert('yep2');
...
$(document).ready(function () {
    ...
    addData = function (data) {
        ...
        /*if (feedCount = 1) {
        $('.widgetHeader:eq(2)').css('background-color', '#ff7200');
        }
        if (feedCount = 2) {
        $('.widgetHeader:eq(2)').css('background-color', '#c55800');
        }
        if (feedCount = 3) {
        $('.widgetHeader:eq(3)').css('background-color', '#a34900');
        }
        if(feedCount = 4) {
        $('.widgetHeader:eq(4)').css('background-color', '#843b00');
        }*/
    ...
    colourSwap = function () {
        ...
        $('.widget:eq(1)').css('background-color', '#ff7200');
        $('.widget:eq(2)').css('background-color', '#c55800');
        $('.widget:eq(3)').css('background-color', '#a34900');
        $('.widget:eq(4)').css('background-color', '#843b00');
    ...
    $('.widgetHeader').live('click', function (event) {
        var $target = $(event.target);
